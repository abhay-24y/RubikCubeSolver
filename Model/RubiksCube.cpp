...
RubiksCube:: we have used as getColorLetter is a static function in the RubiksCube class so we can use is like this.
And
COLOR:: we are using this because COLOR is enum class so it's  Values are scoped and we have to use that syntax.
 */
...
/*
 * We are printing in keeping the linear diagram in mind.
 *                  U       --> first loop.
 *                L F R B   --> second loop print these four.
 *                  D       --> last loop print this.
 *
 */
...
/*
 * randomShuffleCube(times) randomly picks times moves, applies each move to the cube,
   stores all those moves in a vector, and finally returns that vector.
 * srand(time(0)) is used to make sure rand() gives different random numbers each time the program runs.
 * static_cast is used for compile-time safe type conversions.
